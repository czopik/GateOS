...
volatile uint8_t telemetry_zero_req = 0;
volatile uint8_t telemetry_get_req = 0;
...
volatile uint16_t eb_pulse_ms = 0;
volatile uint16_t eb_pulse_lvl = 0;
volatile uint8_t eb_pulse_req = 0;
volatile uint8_t eb_enabled = 0;
volatile uint8_t ascii_help_req = 0;
volatile uint8_t ascii_getcfg_req = 0;
volatile uint8_t ascii_poweroff_req = 0;
#endif
...
void readInputRaw(void) {
    ...
    #endif
    #if defined(CONTROL_SERIAL_USART3)
     /* NOTE: CONTROL_SERIAL_USART3 is used as an input index/priority (e.g. 0 = primary).
       A value of 0 therefore selects the primary input (index 0). This is NOT a "disabled"
       boolean — the code compares `inIdx == CONTROL_SERIAL_USART3` to choose the active source. */
...
#if defined(CONTROL_SERIAL_USART2) || defined(CONTROL_SERIAL_USART3) || defined(FEEDBACK_SERIAL_USART2) || defined(FEEDBACK_SERIAL_USART3)
static void set_ascii_resp(const char *s)
{
  uint16_t i = 0;
  for (; i < (uint16_t)(sizeof(ascii_resp_buf) - 1) && s[i] != '\0'; i++) {
    ascii_resp_buf[i] = s[i];
  }
  ascii_resp_buf[i] = '\0';
  ascii_resp_pending = 1;
}
...
static uint8_t parse_u16(const char *p, uint16_t *out)
{
  uint32_t v = 0;
  uint8_t any = 0;
  while (*p >= '0' && *p <= '9') {
...
/*
 * Process simple ASCII commands on USART (ZERO/GET)
 */
void usart_process_ascii(const uint8_t *data, uint32_t len)
{
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (c == '\n') {
      ...
      const char *p = skip_spaces(cmd_buf);
      char cmd[8] = {0};
      uint8_t ci = 0;
      while (*p != '\0' && *p != ' ' && ci < (uint8_t)(sizeof(cmd) - 1)) {
        cmd[ci++] = *p++;
      }
      cmd[ci] = '\0';
      p = skip_spaces(p);

      // Store last ascii cmd (token only)
      for (uint8_t j = 0; j < (uint8_t)(sizeof(last_ascii_cmd) - 1); j++) {
        last_ascii_cmd[j] = cmd[j];
        if (cmd[j] == '\0') {
          break;
        }
      }
      last_ascii_cmd[sizeof(last_ascii_cmd) - 1] = '\0';

      const uint8_t stopped = (speedAvgAbs <= 5);
      const uint8_t safe_cfg = (!hb_motor_armed && stopped);

      if (cmd[0] == '\0') {
        set_ascii_resp("ERR,syntax\n");
        ...
        telemetry_get_req = 1;
        set_ascii_resp("OK,GET\n");
      } else if (strcmp(cmd, "ZERO") == 0) {
        telemetry_zero_req = 1;
        set_ascii_resp("OK,ZERO\n");
      } else if (strcmp(cmd, "ARM") == 0) {
        if (!stopped) {
          set_ascii_resp("ERR,busy\n");
        } else {
          hb_motor_armed = 1;
          set_ascii_resp("OK,ARM\n");
        }
      } else if (strcmp(cmd, "DISARM") == 0) {
        hb_motor_armed = 0;
        eb_pulse_req = 0;
        set_ascii_resp("OK,DISARM\n");
      } else if (strcmp(cmd, "MODE") == 0) {
        if (!safe_cfg) {
          set_ascii_resp("ERR,busy\n");
        } else if (strcmp(p, "SPD") == 0) {
          ctrlModReqRaw = SPD_MODE;
          ctrlModReq = SPD_MODE;
          set_ascii_resp("OK,MODE SPD\n");
        } else if (strcmp(p, "TRQ") == 0) {
          ctrlModReqRaw = TRQ_MODE;
          ctrlModReq = TRQ_MODE;
          set_ascii_resp("OK,MODE TRQ\n");
        } else if (strcmp(p, "VLT") == 0) {
          ctrlModReqRaw = VLT_MODE;
          ctrlModReq = VLT_MODE;
          set_ascii_resp("OK,MODE VLT\n");
        } else {
          set_ascii_resp("ERR,syntax\n");
        }
      } else if (strcmp(cmd, "CTRL") == 0) {
        (void)p;
        set_ascii_resp("ERR,not_supported\n");
        ...
        if (!safe_cfg) {
          ...
          set_ascii_resp("OK,FW 0\n");
        } else if (strcmp(p, "1") == 0) {
          rtP_Left.b_fieldWeakEna = 1;
          rtP_Right.b_fieldWeakEna = 1;
          set_ascii_resp("OK,FW 1\n");
        } else {
          set_ascii_resp("ERR,syntax\n");
        }
      } else if (strcmp(cmd, "FWMAX") == 0) {
        uint16_t v;
        if (!safe_cfg) {
          set_ascii_resp("ERR,busy\n");
        } else if (!parse_u16(p, &v) || v > 20) {
          set_ascii_resp("ERR,range\n");
        } else {
          rtP_Left.id_fieldWeakMax = (int16_t)v;
          rtP_Right.id_fieldWeakMax = (int16_t)v;
          set_ascii_resp("OK,FWMAX\n");
        }
      } else if (strcmp(cmd, "FWLO") == 0) {
        uint16_t v;
        if (!safe_cfg) {
          set_ascii_resp("ERR,busy\n");
        } else if (!parse_u16(p, &v) || v < 500 || v > 1000 || v >= (uint16_t)rtP_Left.r_fieldWeakHi) {
          set_ascii_resp("ERR,range\n");
        } else {
          rtP_Left.r_fieldWeakLo = (int16_t)v;
          rtP_Right.r_fieldWeakLo = (int16_t)v;
          set_ascii_resp("OK,FWLO\n");
        }
      } else if (strcmp(cmd, "FWHI") == 0) {
        uint16_t v;
        if (!safe_cfg) {
          set_ascii_resp("ERR,busy\n");
        } else if (!parse_u16(p, &v) || v < 1000 || v > 1500 || v <= (uint16_t)rtP_Left.r_fieldWeakLo) {
          set_ascii_resp("ERR,range\n");
        } else {
          rtP_Left.r_fieldWeakHi = (int16_t)v;
          rtP_Right.r_fieldWeakHi = (int16_t)v;
          set_ascii_resp("OK,FWHI\n");
        }
        ...
        if (!safe_cfg) {
          ...
          eb_enabled = 0;
          set_ascii_resp("OK,EB 0\n");
        } else if (strcmp(p, "1") == 0) {
          eb_enabled = 1;
          set_ascii_resp("OK,EB 1\n");
        } else {
          set_ascii_resp("ERR,syntax\n");
        }
      } else if (strcmp(cmd, "EBP") == 0) {
        uint16_t ms;
        uint16_t lvl;
        if (!hb_motor_armed) {
          set_ascii_resp("ERR,disarmed\n");
        } else if (speedAvgAbs > 200) {
          set_ascii_resp("ERR,busy\n");
        } else {
          const char *p2 = p;
          if (!parse_u16(p2, &ms)) {
            ...
            if (!parse_u16(p2, &lvl)) {
              set_ascii_resp("ERR,syntax\n");
            } else if (ms < 50 || ms > 800 || lvl > 500) {
              set_ascii_resp("ERR,range\n");
            } else if (!eb_enabled) {
              set_ascii_resp("ERR,disabled\n");
            } else {
              eb_pulse_ms = ms;
              eb_pulse_lvl = lvl;
              eb_pulse_req = 1;
              set_ascii_resp("OK,EBP\n");
            }
          }
        }
        ...
        if (!safe_cfg) {
          ...
          ascii_poweroff_req = 1;
          set_ascii_resp("OK,POWEROFF\n");
        }
      } else {
        set_ascii_resp("ERR,syntax\n");
      }
    ...
    // ASCII filter: accept only [A-Z0-9 ] plus '\n' above; anything else resets buffer.
    // Any byte <0x20 (except CR/LF handled above) resets; bytes >=0x80 reset.
    if (c < 0x20 || c >= 0x80) {
      cmd_len = 0;
      continue;
    }
...
void usart3_rx_check(void)
{
  ...
  #if defined(CONTROL_SERIAL_USART3) || defined(FEEDBACK_SERIAL_USART3)
  if (pos != old_pos) {
    if (pos > old_pos) {
      usart_process_ascii(&rx_buffer_R[old_pos], pos - old_pos);
    } else {
      usart_process_ascii(&rx_buffer_R[old_pos], rx_buffer_R_len - old_pos);
      if (pos > 0) {
        usart_process_ascii(&rx_buffer_R[0], pos);
      }
    }
  }
  #endif
...
void usart_process_command(SerialCommand *command_in, SerialCommand *command_out, uint8_t usart_idx)
{
  ...
  if (command_in->start == SERIAL_START_FRAME) {
    ...
    *command_out = *command_in;
    /* Update timestamp of last valid binary command.
     * Note: `CONTROL_SERIAL_USART3` can be defined as numeric 0 to indicate
     * the primary input index. The presence of the macro (defined(...))
     * enables serial handling; the numeric value distinguishes primary
     * vs auxiliary input when DUAL_INPUTS is used. Defining it to 0 does
...
void sideboardLeds(uint8_t *leds) {
    ...
    // Error handling
    // Critical error:  LED1 on (RED)     + high pitch beep (handled in main)
    // Soft error:      LED3 on (YELLOW)  + low  pitch beep (handled in main)
    // Consider RIGHT motor error only when right motor is enabled.
