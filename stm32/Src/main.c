...
// - REV_ALLOW_RPM: allow opposite sign only after speed decays below this threshold
#define REV_GUARD_RPM 30
...
int main(void) {
  ...
  // Gate/ESP32 mode: start DISARMED, enforce default SPD request at runtime
  #ifdef VARIANT_USART
    // Gate/USART variant: start DISARMED by default for safety (do not enable motors until explicit ARM)
    hb_motor_armed = 0;
  ...
  while(1) {
    ...
    calcAvgSpeed();                       // Calculate average measured speed: speedAvg, speedAvgAbs
    ...
    #if defined(FEEDBACK_SERIAL_USART2) || defined(FEEDBACK_SERIAL_USART3)
      if (telemetry_zero_req) {
        dist_mm_x1000 = 0;
        dist_mm = 0;
        dist_last_ms = 0;
        telemetry_zero_req = 0;
      }
      {
        int32_t rpm_meas = speedAvg;
        if (ABS(rpm_meas) <= 5) {
          rpm_meas = 0;
        }
        int32_t speed_mm_s = (rpm_meas * MM_PER_REV) / 60;
        uint32_t now_ms = HAL_GetTick();
        uint32_t dt_ms = (dist_last_ms == 0) ? (uint32_t)DELAY_IN_MAIN_LOOP : (now_ms - dist_last_ms);
        // clamp dt to reasonable bounds to avoid spikes
        if (dt_ms < 1) dt_ms = 1;
        if (dt_ms > 200) dt_ms = 200;
        dist_last_ms = now_ms;
        dist_mm_x1000 += speed_mm_s * (int32_t)dt_ms;
        dist_mm = dist_mm_x1000 / 1000;
      }
    #endif
      ...
      // ####### SET OUTPUTS (if the target change is less than +/- 100) #######
      ...
      #ifdef VARIANT_USART
        // Gate mode: single motor (LEFT) + remote ARM/DISARM + failsafe + optional electric brake pulse
        enable = hb_motor_armed ? 1 : 0;

        // Force right channel to 0 in all cases
        #ifndef MOTOR_RIGHT_ENA
          pwmr = 0;
          cmdR = 0;
        #endif

        // Direction reversal zero-cross guard:
        // If requested command sign differs from measured motion sign and speed is still high,
        // apply a small opposite torque brake (TRQ mode) until near standstill,
        // then allow opposite direction command.
        if (hb_motor_armed && !failsafe_active && !eb_pulse_active) {
          int8_t req_sign = 0;
          int8_t mot_sign = 0;
          int16_t req_cmd = input2[inIdx].cmd;

          // Requested sign should be taken from input command (post input processing),
          // not from already-filtered motor setpoint, otherwise reversal is detected too late.
          if (req_cmd > 0) {
            req_sign = 1;
          } else if (req_cmd < 0) {
            req_sign = -1;
          ...
          if (speedAvg > REV_ALLOW_RPM) {
            mot_sign = 1;
          } else if (speedAvg < -REV_ALLOW_RPM) {
            mot_sign = -1;
          }

          if (rev_guard_active) {
            if (speedAvgAbs <= REV_ALLOW_RPM) {
              rev_guard_active = 0;
              ctrlModReqRaw = SPD_MODE;
              ctrlModReq = SPD_MODE;
            } else {
              ...
              ctrlModReq = TRQ_MODE;
              {
                ...
                pwml = (int16_t)(sign * REV_BRAKE_LVL);
                cmdL = pwml;
                pwmr = 0;
                cmdR = 0;
              }
          ...
          if (!rev_guard_active && req_sign != 0 && mot_sign != 0 && req_sign != mot_sign && speedAvgAbs >= REV_GUARD_RPM) {
            rev_guard_active = 1;
            ctrlModReqRaw = TRQ_MODE;
            ctrlModReq = TRQ_MODE;
            {
        ...
        // Failsafe: soft stop / disarm (setpoint=0, PWM off)
        if (!hb_motor_armed || failsafe_active) {
          pwml = 0;
          pwmr = 0;
          cmdL = 0;
          cmdR = 0;
        ...
        // Electric brake pulse (EBP ms level): only when armed and low rpm (validated on RX side too)
        if (eb_pulse_req && hb_motor_armed && !failsafe_active && speedAvgAbs <= 200) {
          eb_pulse_active = 1;
          eb_pulse_end_ms = HAL_GetTick() + (uint32_t)eb_pulse_ms;
          last_eb_ms = eb_pulse_ms;
          last_eb_lvl = eb_pulse_lvl;
          eb_pulse_req = 0;
        }
        if (eb_pulse_active) {
          if (HAL_GetTick() >= eb_pulse_end_ms) {
            eb_pulse_active = 0;
            ctrlModReqRaw = SPD_MODE;
            ctrlModReq = SPD_MODE;
            pwml = 0;
            ...
            ctrlModReqRaw = TRQ_MODE;
            ctrlModReq = TRQ_MODE;
            int16_t lvl = (int16_t)CLAMP((int32_t)last_eb_lvl, 0, 500);
            int16_t sign = 0;
            if (speedAvg > 0) {
              sign = -1;
            } else if (speedAvg < 0) {
    ...
    // ####### TELEMETRY SERIAL OUT (ASCII) #######
    #if defined(FEEDBACK_SERIAL_USART2) || defined(FEEDBACK_SERIAL_USART3)
      {
        uint8_t send_now = 0;
        uint16_t telemetry_ticks = (uint16_t)(TELEMETRY_PERIOD_MS / DELAY_IN_MAIN_LOOP);
        if (telemetry_ticks == 0) {
          telemetry_ticks = 1;
        }
        if (++telemetry_tick >= telemetry_ticks) {
          telemetry_tick = 0;
          send_now = 1;
        }
        if (telemetry_get_req) {
          telemetry_get_req = 0;
          send_now = 1;
        }
        if (send_now) {
          int16_t rpm_meas = speedAvg;
          ...
          if (ctrlModReqRaw == SPD_MODE) {
            mode_str = "SPD";
          } else if (ctrlModReqRaw == TRQ_MODE) {
            mode_str = "TRQ";
          } else if (ctrlModReqRaw == VLT_MODE) {
            mode_str = "VLT";
          }
          ...
          if (len < 0) {
            len = 0;
          } else if (len >= (int)sizeof(tel_buf)) {
            len = (int)sizeof(tel_buf) - 1;
          }


          #if defined(FEEDBACK_SERIAL_USART3)
            if (len > 0) {
              HAL_UART_Transmit(&huart3, (uint8_t *)tel_buf, (uint16_t)len, 10);
            }
    ...
    // ####### POWER BUTTON #######
    // Do not auto poweroff in gate mode; STM32 + UART must keep running.

    // ####### BEEP AND EMERGENCY POWEROFF #######
    if (TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF && speedAvgAbs < 20){  // Overtemperature: DISARM (no full poweroff)
      hb_motor_armed = 0;
      enable = 0;
      beepCount(5, 24, 1);
      ...
      enable = 0;
      beepCount(1, 24, 1);
    } else if (rtY_Left.z_errCode) {                                                                  // 1 beep (low pitch): Motor error, DISARM
      hb_motor_armed = 0;
      enable = 0;
      beepCount(1, 24, 1);
      ...
      // In gate/UART mode the ESP32 may occasionally be busy (WiFi/flash writes) while the motor is already stopped.
      // Avoid nuisance beeps when we're safely stopped and disarmed; still beep if we were moving / armed.
      if (speedAvgAbs >= 20 || hb_motor_armed || ABS(cmdL) > 50 || ABS(cmdR) > 50) {
        beepCount(3, 24, 1);
      } else {
        beepCount(0, 0, 0);
      }
    ...
    /* Inactivity poweroff guard: only act if INACTIVITY_TIMEOUT > 0.
     * The macro may be set to 0 to disable auto poweroff due to inactivity.
     */
    if (INACTIVITY_TIMEOUT > 0) {
      if (inactivity_timeout_counter > (INACTIVITY_TIMEOUT * 60 * 1000) / (DELAY_IN_MAIN_LOOP + 1)) {  // rest of main loop needs maybe 1ms
        hb_motor_armed = 0;   // Prefer DISARM over full poweroff
        enable = 0;
      }
    }
