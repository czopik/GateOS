...
// Limitation settings
// --- Gate / bench-friendly defaults ---
// For a sliding gate the mechanical load can spike (ice, friction, hard stops).
// Start conservatively on the bench and increase only if needed.
#define I_MOT_MAX       20              // [A] Maximum single motor current limit (bench-safe)
#define I_DC_MAX        30              // [A] DC link current limit (keep ~I_MOT_MAX + 3A)
#define N_MOT_MAX       500             // [rpm] Maximum motor speed limit (bench-safe)
...
// ========== PARAMETRY SILNIKA EPS BLDC ==========
// KRYTYCZNIE WAŻNE: Musisz znać te wartości dla swojego silnika!

#define MOTOR_POLE_PAIRS    8       // LICZBA PAR BIEGUNÓW - MUSISZ TO SPRAWDZIĆ!
                                        // Typowe wartości dla EPS: 4, 5, 6, 7, 8
                                        // JAK SPRAWDZIĆ:
                                        // 1. Obracaj wał ręcznie i licz "kliknięcia" magnetyczne
                                        // 2. Podziel liczbę kliknięć przez 2
                                        // Przykład: 14 kliknięć = 7 par biegunów

// Rezystancja i indukcyjność faz (opcjonalne dla podstawowego FOC)
// Możesz zmierzyć rezystancję multimetrem między dwoma fazami
 #define MOTOR_PHASE_RESISTANCE  0.5f   // [Ohm] Typowo 0.3-1.0 Ohm dla EPS
 #define MOTOR_PHASE_INDUCTANCE  100.0f // [μH] Typowo 50-200 μH dla EPS

// Konfiguracja czujników Halla
// UWAGA: Jeśli silnik nie działa płynnie, sprawdź kolejność podłączenia Halli!
// Standardowe mapowanie pinów dla hoverboard:
// Hall A -> PA15
// Hall B -> PB3
// Hall C -> PB10
...
// ############################### TELEMETRY ###############################
#define MM_PER_REV             140     // [mm/rev] Gate travel per motor revolution
#define TELEMETRY_PERIOD_MS     50     // [ms] Telemetry send period (20 Hz)
// UART/command dropouts can happen when the ESP32 is briefly busy (WiFi/LittleFS).
// Relax the failsafe timers to avoid nuisance beeps/stop-start on the bench.
#define FAILSAFE_CMD_MS       1000     // [ms] No binary cmd -> setpoint=0 (was 300)
#define DISARM_CMD_MS         2000     // [ms] No binary cmd -> DISARM (was 1500)
// ########################### END OF TELEMETRY ############################
...
// ############################ VARIANT_USART SETTINGS ############################
#ifdef VARIANT_USART
  #undef CTRL_MOD_REQ
  #define CTRL_MOD_REQ          SPD_MODE  // Speed control for UART variant
  #undef MOTOR_RIGHT_ENA                  // Single-motor setup: disable right channel
  // Gate mode safety: never cut OFF_PIN power (keep 15V alive)
  #define KEEP_POWER_ON_ALWAYS 1
  // Fault recovery: hold motors off for cooldown after fault/overtemp/bat-dead (ms)
  #define FAULT_COOLDOWN_MS 4000
  ...
  // NOTE: Here CONTROL_SERIAL_USART3 is set to 0 intentionally. In this codebase the value
  // represents the input index / priority (0 = primary input). It is NOT a disable flag.
  // Keep FEEDBACK_SERIAL_USART3 enabled to transmit telemetry on USART3.
  #define FEEDBACK_SERIAL_USART3      // right sensor board cable, disable if I2C (nunchuk or lcd) is used!
...
// AUTO-RESOLVE SERIAL CONFLICTS (needed when build_flags define DEBUG/FEEDBACK together)
#if defined(FEEDBACK_SERIAL_USART3) && defined(DEBUG_SERIAL_USART3)
  #undef DEBUG_SERIAL_USART3
#endif
