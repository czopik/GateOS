...
void MotorController::setDuty(int duty) {
  if (driverKind == DRIVER_HOVER_UART) {
    ...
    bool forward = motionActive ? motionDirectionForward : directionForward;
...
void MotorController::tick(uint32_t nowMs, float currentDistance) {
  ...
  if (remaining_eff < 0.60f) {
    float ratio = remaining_eff / 0.60f;
    if (ratio < 0.0f) ratio = 0.0f;
    if (ratio > 1.0f) ratio = 1.0f;
    int fineTarget = (int)(targetSpeed * ratio);
    int damp = (int)(fabsf(vEst) * 0.25f * (float)targetSpeed);
    fineTarget -= damp;
    int minFine = minSpeed;
    if (driverKind == DRIVER_HOVER_UART) {
      minFine = 0;
    }
    if (fineTarget < minFine) fineTarget = minFine;
    if (fineTarget < desired) desired = fineTarget;
  }
