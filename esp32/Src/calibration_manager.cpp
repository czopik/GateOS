...
namespace {
static int parsePullModeToPinMode(const String& pullMode) {
  if (pullMode == "down") return INPUT_PULLDOWN;
  if (pullMode == "none") return INPUT;
  return INPUT_PULLUP;
}
} // namespace
...
bool CalibrationManager::start() {
  ...
  // CALIB_FIX: use calibrated runtime sources (limits/sensors config) so
  // calibration sees the same electrical logic as /api/status and input handling.
  initProbe(limitOpen,
            cfg->limitsConfig.open.pin,
            cfg->limitsConfig.open.invert,
            cfg->limitsConfig.open.pullMode);
  initProbe(limitClose,
            cfg->limitsConfig.close.pin,
            cfg->limitsConfig.close.invert,
            cfg->limitsConfig.close.pullMode);
  initProbe(obstacle,
            cfg->sensorsConfig.photocell.pin,
            cfg->sensorsConfig.photocell.invert,
            cfg->sensorsConfig.photocell.pullMode);

  if (gate) gate->stop();
  stopMotor();
  motor->setInvertDir(motorInvertOriginal);
  Serial.printf("CAL: start direction source=motor.invertDir original=%d gpio.dirInvert(legacy_ignored)=%d applied=%d\n",
                motorInvertOriginal ? 1 : 0,
                cfg->gpioConfig.dirInvert ? 1 : 0,
                motorInvertOriginal ? 1 : 0);
...
bool CalibrationManager::confirmDirection(bool invert) {
  ...
  if (motor) {
    motor->setInvertDir(invert);
  }
  setStep(CAL_MOVE_TO_CLOSE, "Ustawianie pozycji startowej");
  Serial.printf("CAL: direction confirmed invert=%d gpio.dirInvert(legacy_ignored)=%d\n",
                invert ? 1 : 0,
                cfg->gpioConfig.dirInvert ? 1 : 0);
  return true;
}
...
void CalibrationManager::initProbe(InputProbe& probe, int pin, bool invert, const String& pullMode) {
  probe.pin = pin;
  probe.present = pin >= 0;
  probe.detected = false;
  probe.invert = invert;
  probe.pullMode = pullMode;
  probe.idleState = -1;
  probe.startMs = millis();
  if (pin >= 0) {
    // CALIB_FIX: honor configured pull mode to avoid false active limit states.
    pinMode(pin, parsePullModeToPinMode(pullMode));
...
void CalibrationManager::determineDirection() {
  ...
  if (forwardHitsOpen || reverseHitsClose || toggle) {
    ...
    motor->setInvertDir(motorInvertProposed);
    Serial.printf("CAL: motor invert=%d (orig=%d toggle=%d gpio.dirInvert(legacy_ignored)=%d)\n",
...
void CalibrationManager::fillStatus(JsonObject& out) const {
  ...
  // CALIB_FIX: expose manual-calibration contract fields for UI/API compatibility.
  out["enabled"] = true;
  out["active"] = running;
  out["closeConfirmed"] = (step >= CAL_MOVE_TO_OPEN) || (step == CAL_COMPLETE);
  out["openConfirmed"] = (step >= CAL_RAMP_TEST) || (step == CAL_COMPLETE);
  ...
  long travelMm = 0;
  if (travelPulses > 0 && cfg &&
      cfg->gateConfig.pulsesPerRevolution > 0 &&
      cfg->gateConfig.wheelCircumference > 0.0f) {
    travelMm = lroundf(((float)travelPulses * cfg->gateConfig.wheelCircumference /
                        (float)cfg->gateConfig.pulsesPerRevolution) * 1000.0f);
  }
  out["travelMm"] = travelMm;
