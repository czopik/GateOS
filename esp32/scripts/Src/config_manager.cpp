...
void ConfigManager::buildJson(JsonDocument& doc) const {
  ...
  JsonObject hall = sensors.createNestedObject("hall");
  hall["enabled"] = sensorsConfig.hall.enabled;
  hall["pin"] = sensorsConfig.hall.pin;
  hall["invert"] = sensorsConfig.hall.invert;
  hall["pullMode"] = sensorsConfig.hall.pullMode;
  hall["debounceMs"] = sensorsConfig.hall.debounceMs;
...
if (obj.containsKey("motor")) {
  ...
  if (obj.containsKey("sensors")) {
    ...
    if (!sensors.isNull()) {
      JsonObjectConst hall = sensors["hall"];
      if (!hall.isNull()) {
        sensorsConfig.hall.enabled = hall["enabled"] | sensorsConfig.hall.enabled;
        sensorsConfig.hall.pin = hall["pin"] | sensorsConfig.hall.pin;
        sensorsConfig.hall.invert = hall["invert"] | sensorsConfig.hall.invert;
        sensorsConfig.hall.pullMode = String((const char*)(hall["pullMode"] | sensorsConfig.hall.pullMode.c_str()));
        sensorsConfig.hall.debounceMs = hall["debounceMs"] | sensorsConfig.hall.debounceMs;
      }
...
bool ConfigManager::validate(JsonVariantConst root, String& error) {
  ...
  if (obj.containsKey("sensors")) {
    ...
    if (!sensors.isNull()) {
      ...
      JsonObjectConst hall = sensors["hall"];
      if (!hall.isNull()) {
        int pin = hall["pin"] | sensorsConfig.hall.pin;
        int debounce = hall["debounceMs"] | (int)sensorsConfig.hall.debounceMs;
        const char* pull = hall["pullMode"] | "";
        if (pin >= 0 && isForbiddenPin(pin)) {
          error = "sensors.hall.forbidden_pin";
          return false;
        }
        if (!pullOk(pull)) {
          error = "sensors.hall.pull_invalid";
          return false;
        }
        if (debounce < 0 || debounce > 2000) {
          error = "sensors.hall.debounce_out_of_range";
          return false;
        }
      }
