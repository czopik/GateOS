...
public:
  ...
  void setForceDecelBoost(bool enable);
  ...
  uint32_t hoverRxLines() const { return hover.getRxLines(); }
  uint32_t hoverRxTelLines() const { return hover.getRxTelLines(); }
  uint32_t hoverRxBadLines() const { return hover.getRxBadLines(); }
...
private:
  ...
  bool forceDecelBoost = false;
  int8_t motionBand = -1; // 0=precision,1=approach,2=far
  uint32_t vLastTickMs = 0;
  float vLastDist = 0.0f;
  float vEst = 0.0f;
  ...
  int decelSpeed(float remaining, int maxSpeed, float brakingDistance);
};
